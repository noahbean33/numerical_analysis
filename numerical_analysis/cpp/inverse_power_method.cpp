...
int main()
{
   ...
   if (OK) {
      ...
      if (OK) {
         ...
         LP = 1;
         for (I=2; I<=N; I++) 
            if (fabs(X[I-1]) > fabs(X[LP-1])) LP = I;
         /* STEP 4 */
         AMAX = X[LP-1];
         ...
         while ((K <= NN) && OK) {
            ...
            LP = 1;
            for (I=2; I<=N; I++) 
               if (fabs(Y[I-1]) > fabs(Y[LP-1])) LP = I;
            AMAX = Y[LP-1];
            ERR = 0.0;
            for (I=1; I<=N; I++) {
               T = Y[I-1] / AMAX;
   ...
   *OK = true;
   ...
   while ((I <= M) && OK) {
      ...
      for (IP=J; IP<=N; IP++) {
         L1 = NROW[IMAX-1];
         L2 = NROW[IP-1];
         if (fabs(A[L2-1][I-1]) > fabs(A[L1-1][I-1])) IMAX = IP;
      }  
      if (fabs(A[NROW[IMAX-1]-1][I-1]) <= ZERO) {
         *OK = false;
