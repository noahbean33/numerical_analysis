...
double G2(double X, double Y)
{
   ...
   T = 1.0e-05;
   Z1 = 0.0;
   if ( ( (0.2-T) <= X) && (X <= (0.4+T)) && (fabs(Y-0.2) <= T)) 
      Z1 = X;
   if ( ( (0.5-T) <= X) && (X <= (0.6+T)) && (fabs(Y-0.1) <= T))
      Z1 = X;
   if ( ( -T <= Y) && (Y <= (0.1+T)) && (fabs(X-0.6) <= T)) 
      Z1 = Y;
   if ( ( -T <= X) && (X <= (0.2+T)) && (fabs(Y+X-0.4) <= T)) 
      Z1 = (X+Y)/sqrt(2.0);
   if ( (0.4 -T <= X) && (X <= (0.5+T)) && (fabs(Y+X-0.6) <= T)) 
      Z1 = (X+Y)/sqrt(2.0);
   g2 = Z1;
   return g2;
}
...
double QQ(int L, double *XX, double *YY, double DELTA, int J1, int J2, int J3, int I1, int I2, double A[][3], double B[][3], double C[][3])
{
   ...
   T3 = S[6];
   QQQ = 0.5*(T1/20.0 + 2.0*T2/15.0 + 9.0*T3/20.0)*fabs(DELTA);
   qq = QQQ;
   return qq;
}
