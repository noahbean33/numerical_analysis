...
int main()
{
   ...
   if (OK) {
	 ...
	 Gauss elimination will begin */
      ...
      while ((I <= N-1) && OK) {
	 KK = I;
	 while ((fabs(A[KK-1][I-1]) < ZERO) && (KK <= N))  
	    KK++;
	 if (KK > N) {
	    OK = false;
	 ...
	 for (I=1; I<=N; I++) {
	    ...
	    if (fabs(X[I-1]) > TOL) LL = 1; 
	    if (K == 1) {
	       if (fabs(X[I-1]) > YMAX) 
		  YMAX = fabs(X[I-1]);
	       if (fabs(XX[I-1]) > XXMAX)
		  XXMAX = fabs(XX[I-1]);
	    }
	    TEMP = XX[I-1];
	    XX[I-1] = CHIP(RND,D,XX[I-1]+X[I-1]);
	    TEMP = fabs(TEMP-XX[I-1]);
